...
class TwitterTelegramService:
        ...
        for account in self.config.accounts:
            ...
            for post in sorted(posts, key=lambda item: int(item.id)):
                ...
                try:
                    ...
                    result.errors += 1
                    result.status_lines.append(f"Error sending @{account.username} status {post.id}: {exc}")
                    break
                result.sent += 1
                method = getattr(delivery, "method", "sent")
                fallback = " with fallback" if getattr(delivery, "fallback_used", False) else ""
                self.state.mark_sent(post.id, username=account.username, delivery_status=f"{method}{fallback}")
                result.status_lines.append(f"Sent @{account.username} status {post.id} via {method}")
                latest_seen = candidate_seen
            if latest_seen:
    ...
    def run_forever(self, *, dry_run: bool = False) -> None:
        while True:
            ...
            time.sleep(self._sleep_seconds(result))
...
def build_service(
    *,
    ...
    db_path = state_path or config.state.path
